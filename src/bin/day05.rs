...
pub fn main() {
    ...
    let page_lines = pages.lines().skip(2).map(|line| line.split(",").map(|page| page.parse::<i32>().unwrap()).collect::<Vec<_>>());
    ...
    for pages in page_lines {
        ...
        'page: for (i, page) in pages.iter().enumerate() {
            for (before, after) in rules.iter().filter(|(before, after)| pages.contains(before) && pages.contains(after)) {
        ...
        if is_sorted {
            ...
            sorted.sort_by(|a, b| {
                let rule = rules.iter().find(|(before, after)| a == before && b == after);
